...
| Projects  | `projects/`  | `features/projects/`   | Cannot delete if linked movements exist. `managerId` is a required FK to `User` (not Staff, not a plain string). Manager only needs to exist in the `users` table — no Supabase Auth required. |
