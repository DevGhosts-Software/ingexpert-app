...
#### Scenario: Authenticated user role can create projects

- **WHEN** an authenticated user with `user` role attempts to create a project
- **THEN** the RLS policy SHALL allow the INSERT operation
- **THEN** the project SHALL be created and synced to server
- **AND** update/delete operations SHALL remain admin-only

#### Scenario: Non-admin user cannot update or delete projects

- **WHEN** an authenticated user with `user` role attempts to update a project
- **THEN** the RLS policy SHALL block the UPDATE operation
- **AND** attempts to delete a project SHALL also be blocked
