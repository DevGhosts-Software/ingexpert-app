...
#### Scenario: User role can INSERT items but not UPDATE or DELETE

- **WHEN** an authenticated user with `user` role attempts to INSERT an item
...
#### Scenario: Admin can perform all item operations

- **WHEN** an authenticated user with admin role attempts item operations
- **THEN** INSERT, UPDATE, and DELETE operations SHALL all be allowed
