...
## Requirement: Movement stats migration SHALL enforce parity gates

Before movement stats are switched to local-primary computation, parity checks against API semantics MUST be run with explicit mismatch telemetry.

#### Scenario: Movement stats dual-run validation

- **WHEN** movement stats are evaluated for local-first cutover
- **THEN** local and API aggregates (`total`, `purchases`, `returns`, `exits`, `writeoffs`, `thisMonth`) MUST be compared
- **THEN** cutover MUST remain blocked until parity acceptance criteria are met
